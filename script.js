...
const player = { x: 100, y: groundY - 40, w: 28, h: 40, vx: 0, vy: 0, onGround: false, facing: 1 };

const level = {
  clouds: [
    { x: 120, y: 70 }, { x: 430, y: 66 }, { x: 740, y: 75 }, { x: 1080, y: 68 },
    { x: 1470, y: 80 }, { x: 1880, y: 70 }, { x: 2330, y: 66 }, { x: 2780, y: 75 },
    { x: 3300, y: 72 }, { x: 3730, y: 64 }, { x: 4190, y: 79 }, { x: 4680, y: 70 },
    { x: 5180, y: 65 }, { x: 5620, y: 78 },
  ],
  bushes: [
    { x: 220, w: 95 }, { x: 840, w: 120 }, { x: 1650, w: 90 }, { x: 2490, w: 120 },
    { x: 3420, w: 92 }, { x: 4380, w: 130 }, { x: 5260, w: 105 },
  ],
  hills: [
    { x: 40, w: 120, h: 70 }, { x: 1160, w: 140, h: 85 }, { x: 2040, w: 110, h: 75 },
    { x: 2980, w: 150, h: 95 }, { x: 3940, w: 120, h: 80 }, { x: 5500, w: 130, h: 90 },
  ],
  blocks: [
    { x: 320, y: 300, kind: "question" }, { x: 352, y: 300, kind: "brick" }, { x: 384, y: 300, kind: "question" },
    { x: 1040, y: 270, kind: "question" }, { x: 1072, y: 270, kind: "brick" },
    { x: 1180, y: 220, kind: "brick" }, { x: 1212, y: 220, kind: "brick" }, { x: 1244, y: 220, kind: "brick" },
    { x: 1550, y: 300, kind: "question" }, { x: 1582, y: 300, kind: "question" },
    { x: 1880, y: 250, kind: "brick" },
    { x: 2360, y: 270, kind: "question" }, { x: 2392, y: 270, kind: "brick" }, { x: 2424, y: 270, kind: "question" },
    { x: 3200, y: 300, kind: "brick" }, { x: 3232, y: 300, kind: "brick" },
    { x: 4010, y: 250, kind: "question" },
  ],
  pipes: [
    { x: 520, h: 48 }, { x: 770, h: 64 }, { x: 980, h: 96 }, { x: 1330, h: 64 },
    { x: 2140, h: 80 }, { x: 3650, h: 50 }, { x: 4700, h: 70 },
  ],
  stairs: [
    { x: 2840, steps: 6 }, { x: 4380, steps: 8 },
  ],
};
...
function drawCloud(x, y) {
  rect(x, y + 8, 48, 16, palette.cloud);
  rect(x + 10, y, 28, 16, palette.cloud);
  rect(x + 20, y + 4, 24, 14, "#d8efff");
}

function drawBrick(x, y) {
  rect(x, y, TILE, TILE, palette.brickA);
  rect(x + 2, y + 2, TILE - 4, 3, palette.brickB);
  rect(x + 2, y + 8, TILE - 4, 2, palette.brickB);
  rect(x + 2, y + 12, TILE - 4, 2, palette.brickB);
}

function drawQuestion(x, y) {
  rect(x, y, TILE, TILE, palette.blockQ);
  rect(x + 2, y + 2, TILE - 4, TILE - 4, palette.blockQ2);
...
function drawHill(x, w, h) {
  const y = groundY - h;
  rect(x, y + 18, w, h - 18, palette.hill);
  rect(x + 10, y + 10, w - 20, 20, palette.hill);
  rect(x + 28, y + 25, 8, 8, palette.hillShade);
  rect(x + w - 40, y + 34, 8, 8, palette.hillShade);
}

function drawBush(x, w) {
  const y = groundY - 22;
  rect(x, y, w, 22, palette.bush);
  rect(x + 8, y - 10, w - 16, 12, palette.bush);
  rect(x + 20, y + 6, 12, 6, palette.bushShade);
}

function drawStairs(baseX, steps) {
  for (let i = 0; i < steps; i++) {
    for (let j = 0; j <= i; j++) drawBrick(baseX + i * TILE, groundY - TILE * (j + 1));
  }
}

function drawBackground() {
  rect(0, 0, canvas.width, canvas.height, palette.sky);
  level.clouds.forEach((c) => drawCloud(c.x - cameraX * 0.35, c.y));
}

function drawGround() {
  rect(-cameraX, groundY, worldWidth, canvas.height - groundY, palette.brickA);
  for (let x = 0; x < worldWidth; x += TILE) drawBrick(x - cameraX, groundY);
}
...
function drawWorld() {
  ...
  level.pipes.forEach((p) => drawPipe(p.x - cameraX, p.h));
  level.stairs.forEach((s) => drawStairs(s.x - cameraX, s.steps));

  // End-area inspired flag + mini castle
...
function drawPlayer() {
  ...
  rect(x + (player.facing === 1 ? 19 : 1), player.y + 11, 4, 4, "#1b130f");
}
...
function resolveCollisions() {
  ...
  level.blocks.forEach((b) => {
    ...
    if (overlap(player, r)) {
      const wasAbove = player.y + player.h - player.vy <= r.y + 2;
      const wasBelow = player.y - player.vy >= r.y + r.h - 2;
      if (player.vy >= 0 && wasAbove) { player.y = r.y - player.h; player.vy = 0; player.onGround = true; }
      else if (player.vy < 0 && wasBelow) {
        player.y = r.y + r.h;
        player.vy = 0.4;
        if (b.kind === "question") {
          b.kind = "brick";
          for (let i = 0; i < 3; i++) state.particles.push({ x: b.x + 5 + i * 4, y: b.y - 2, t: 70 });
          logEvent("Question block hit.");
        }
      }
    }
  });
  ...
  level.pipes.forEach((p) => {
    ...
    if (overlap(player, r)) {
      ...
      if (player.vx < 0) player.x = r.x + r.w;
      player.vx = 0;
    }
  });
  ...
  level.stairs.forEach((s) => {
    for (let i = 0; i < s.steps; i++) {
      for (let j = 0; j <= i; j++) {
        ...
        if (overlap(player, r) && player.vy >= 0 && player.y + player.h - player.vy <= r.y + 2) {
...
function drawParticles() {
  state.particles.forEach((p) => rect(p.x - cameraX, p.y, 4, 6, "#ffd84f"));
}
...
function randomEvent() {
  const events = [
    ...
    () => {
      ...
      for (let i = 0; i < burst; i++) state.particles.push({ x: player.x + i * 8, y: player.y - 10, t: 120 });
      logEvent(`Coin burst x${burst}.`);
    },
  ];
...
function resetWorld() {
  ...
  level.blocks.forEach((b, i) => {
    const originalKinds = ["question","brick","question","question","brick","brick","brick","brick","question","question","brick","question","brick","question","brick","brick","question"];
    b.kind = originalKinds[i] || "brick";
  });
  state.particles = [];
  seedEnemies();
...
jumpControl.addEventListener("input", () => {
  state.jumpForce = Number(jumpControl.value);
  jumpValue.textContent = state.jumpForce;
});
...
openCustomizer.addEventListener("click", () => {
  ...
  openCustomizer.setAttribute("aria-expanded", "true");
});
closeCustomizer.addEventListener("click", () => {
  customizerPanel.classList.add("hidden");
  openCustomizer.setAttribute("aria-expanded", "false");
});
